...
public:
  ...
  void computePoints() {
    ...
    for (int i = sectors/2; i <= sectors; i++) {
      double x1 = radius * cos(angle);
      double y1 = radius * sin(angle);
  ...
  HalfCylinder(double radius, double height, int sectors) {
    this->radius = radius;
    this->height = height;
    this->sectors = sectors;
    computePoints();
...
public:
  ...
  HalfSphere(double radius, int sectors, int stacks) {
    this->radius = radius;
    this->sectors = sectors;
    this->stacks = stacks;
...
void HalfSphere::compute_circular_unit_vector() {
  // from -1 to 1, divide into sectors, same for stacks
  double sector_step = 2.0 / sectors;
  double stack_step = 2.0 / stacks;
  for (int i = 0; i <= this->stacks/2; i++) {
    circular_unit_vectors.push_back(std::vector<Point3D>());

    double y = 0.0 + i * stack_step;
    for (int j = 0; j <= this->sectors; j++) {
      double x = -1.0 + j * sector_step;
      double z = 1.0;
...
void HalfSphere::compute_points_on_updated_radius() {
  points.clear();
  for (int i = 0; i < this->circular_unit_vectors.size(); i++) {
    points.push_back(std::vector<Point3D>());
    for (int j = 0; j < this->circular_unit_vectors[i].size(); j++) {
